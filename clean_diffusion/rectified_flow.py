#!/usr/bin/env python3
"""Rectified Flow + RF Euler + CFG 的端到端教学实现。

这个文件故意采用 CleanRL-style 单文件结构：读者打开一个文件，就能顺着
配置 -> 数据 -> 模型 -> Rectified Flow objective -> RF Euler sampler -> train/sample 主流程读完。

当前文件实现阶段 2：

- Rectified Flow velocity objective
- RF Euler ODE sampler
...
python clean_diffusion/rectified_flow.py train \
...
python clean_diffusion/rectified_flow.py sample \
  --ckpt-root checkpoints/rf_smoke \
  --clip-root /path/to/clip \
  --output-dir samples/rf_smoke \
  --image-size 32 \
  --base-channels 16 \
  --channel-mults 1,2,4 \
  --scheduler rf_euler \
...
Rectified Flow 第一版的核心训练公式：
  ...
  x_t = (1 - t) * x1 + t * x0
  v_target = x0 - x1
  loss = MSE(v_theta(x_t, t, text), v_target)

采样时从 Gaussian noise 出发，沿 rectified velocity field 用 RF Euler 积分到 t=1。第一版先与 FM 使用相同 loss，后续可加入 reflow / straightening。
"""
...
def _early_help_if_requested() -> None:
    ...
    if len(sys.argv) == 1 or sys.argv[1] in {"-h", "--help"}:
        print(
            "CleanDiffusion Rectified Flow single-file baseline\n\n"
            "Usage:\n"
            "  python clean_diffusion/rectified_flow.py train [options]\n"
            "  python clean_diffusion/rectified_flow.py sample [options]\n\n"
            "Run subcommand help:\n"
            "  python clean_diffusion/rectified_flow.py train --help\n"
            "  python clean_diffusion/rectified_flow.py sample --help"
        )
        raise SystemExit(0)
    if len(sys.argv) >= 3 and sys.argv[2] in {"-h", "--help"}:
        if sys.argv[1] == "train":
            print(
                ...
                "  --max-steps 1 --sample-scheduler rf_euler --sample-steps 4 --no-amp"
            )
            raise SystemExit(0)
        if sys.argv[1] == "sample":
            print(
                "Sample from Rectified Flow checkpoint\n\n"
                "Required:\n"
                "  --ckpt-root PATH\n"
                "  --clip-root PATH\n\n"
                "Common:\n"
                "  --output-dir samples --image-size 32 --base-channels 16 --channel-mults 1,2,4\n"
                "  --scheduler rf_euler --steps 4 --guidance-scales 3.0 --prompts \"a small red car\""
            )
            raise SystemExit(0)
...
class TrainConfig:
    ...
    def resolved_run_name(self) -> str:
        ...
        return sanitize_name(f"rf_unet_{time.strftime('%Y%m%d_%H%M%S')}{suffix}")
...
# =============================================================================
# 5. Rectified Flow objective：第一版保持直线路径速度场学习。
# =============================================================================
...
def rectified_flow_loss(model: nn.Module, x0: torch.Tensor, text_context: torch.Tensor) -> torch.Tensor:
    """第一版学习从 noise 指向 data 的直线路径速度场。

    x0 是真实图像，x1 是 Gaussian noise。我们在直线路径
    x_t = (1 - t) * x1 + t * x0 上随机采样时间 t，并让模型预测
    v_target = x0 - x1。
    """
    b = x0.size(0)
    x1 = torch.randn_like(x0)
    t = torch.rand(b, device=x0.device)
    t_view = expand_time(t, x0)
...
# =============================================================================
# 6. CFG + RF Euler sampler：采样沿 rectified velocity field 从噪声走到数据。
# =============================================================================
...
def sample_with_scheduler(
    ...
    model.eval()
    if scheduler == "rf_euler":
        return rf_euler_sample(model, device, cond_context, uncond_context, guidance_scale, steps, shape)
    raise ValueError(f'scheduler should be "rf_euler", got {scheduler}')
...
def train(cfg: TrainConfig) -> None:
    if cfg.sample_scheduler != "rf_euler":
        raise ValueError(f'当前 Rectified Flow 文件只支持 sample_scheduler="rf_euler"，got {cfg.sample_scheduler}')
        ...
        for epoch in range(cfg.epochs):
            for images, text_tokens, captions, cond_captions, is_dropped in dataloader:
                ...
                with torch.amp.autocast(device_type=device.type, enabled=cfg.amp and device.type == "cuda"):
                    loss = rectified_flow_loss(model, images, text_context)
...
def sample(cfg: SampleConfig) -> None:
    if cfg.scheduler != "rf_euler":
        raise ValueError(f'当前 Rectified Flow 文件只支持 scheduler="rf_euler"，got {cfg.scheduler}')
    device = get_device(cfg.device)
...
def add_train_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--sample-scheduler", choices=["rf_euler"], default="rf_euler")
    ...
    parser.add_argument("--sample-eta", type=float, default=0.0, help="保留参数位；FM Euler 当前不使用 eta")
...
def add_sample_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--scheduler", choices=["rf_euler"], default="rf_euler")
    parser.add_argument("--steps", type=int, default=50)
    parser.add_argument("--eta", type=float, default=0.0, help="保留参数位；FM Euler 当前不使用 eta")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="CleanDiffusion Rectified Flow single-file baseline")
    subparsers = parser.add_subparsers(dest="command", required=True)
    train_parser = subparsers.add_parser("train", help="train Rectified Flow")
    add_train_args(train_parser)
    sample_parser = subparsers.add_parser("sample", help="sample from a Rectified Flow checkpoint")
