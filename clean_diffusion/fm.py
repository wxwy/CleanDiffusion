#!/usr/bin/env python3
"""Flow Matching + Euler + CFG 的端到端教学实现。

这个文件故意采用 CleanRL-style 单文件结构：读者打开一个文件，就能顺着
配置 -> 数据 -> 模型 -> Flow Matching objective -> Euler sampler -> train/sample 主流程读完。
...
python clean_diffusion/fm.py train \
...
python clean_diffusion/fm.py sample \
  --ckpt-root checkpoints/fm_smoke \
  --clip-root /path/to/clip \
  --output-dir samples/fm_smoke \
  --image-size 32 \
  --base-channels 16 \
  --channel-mults 1,2,4 \
  --scheduler euler \
...
采样时从 Gaussian noise 出发，沿速度场用 Euler ODE 积分到 t=1。
"""
...
def _early_help_if_requested() -> None:
    ...
    if len(sys.argv) == 1 or sys.argv[1] in {"-h", "--help"}:
        print(
            "CleanDiffusion Flow Matching single-file baseline\n\n"
            "Usage:\n"
            "  python clean_diffusion/fm.py train [options]\n"
            "  python clean_diffusion/fm.py sample [options]\n\n"
            "Run subcommand help:\n"
            "  python clean_diffusion/fm.py train --help\n"
            "  python clean_diffusion/fm.py sample --help"
        )
        raise SystemExit(0)
    if len(sys.argv) >= 3 and sys.argv[2] in {"-h", "--help"}:
        if sys.argv[1] == "train":
            print(
                ...
                "  --max-steps 1 --sample-scheduler euler --sample-steps 4 --no-amp"
            )
            raise SystemExit(0)
        if sys.argv[1] == "sample":
            print(
                "Sample from Flow Matching checkpoint\n\n"
                "Required:\n"
                "  --ckpt-root PATH\n"
                "  --clip-root PATH\n\n"
                "Common:\n"
                "  --output-dir samples --image-size 32 --base-channels 16 --channel-mults 1,2,4\n"
                "  --scheduler euler --steps 4 --guidance-scales 3.0 --prompts \"a small red car\""
            )
            raise SystemExit(0)
...
class TrainConfig:
    ...
    def resolved_run_name(self) -> str:
        ...
        return sanitize_name(f"fm_unet_{time.strftime('%Y%m%d_%H%M%S')}{suffix}")
...
# =============================================================================
# 5. Flow Matching objective：这是训练时最核心的算法部分。
# =============================================================================
...
def flow_matching_loss(model: nn.Module, x0: torch.Tensor, text_context: torch.Tensor) -> torch.Tensor:
    """学习从 noise 指向 data 的常速度场。
...
# =============================================================================
# 6. CFG + Euler sampler：训练学速度场，采样沿 ODE 从噪声走到数据。
# =============================================================================
...
def sample_with_scheduler(
    ...
    model.eval()
    if scheduler == "euler":
        return euler_sample(model, device, cond_context, uncond_context, guidance_scale, steps, shape)
    raise ValueError(f'scheduler should be "euler", got {scheduler}')
...
def train(cfg: TrainConfig) -> None:
    if cfg.sample_scheduler != "euler":
        raise ValueError(f'当前 FM 文件只支持 sample_scheduler="euler"，got {cfg.sample_scheduler}')
        ...
        for epoch in range(cfg.epochs):
            for images, text_tokens, captions, cond_captions, is_dropped in dataloader:
                ...
                with torch.amp.autocast(device_type=device.type, enabled=cfg.amp and device.type == "cuda"):
                    loss = flow_matching_loss(model, images, text_context)
...
def sample(cfg: SampleConfig) -> None:
    if cfg.scheduler != "euler":
        raise ValueError(f'当前 FM 文件只支持 scheduler="euler"，got {cfg.scheduler}')
    device = get_device(cfg.device)
...
def add_train_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--sample-scheduler", choices=["euler"], default="euler")
...
def add_sample_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--scheduler", choices=["euler"], default="euler")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="CleanDiffusion Flow Matching single-file baseline")
    subparsers = parser.add_subparsers(dest="command", required=True)
    train_parser = subparsers.add_parser("train", help="train Flow Matching")
    add_train_args(train_parser)
    sample_parser = subparsers.add_parser("sample", help="sample from a Flow Matching checkpoint")
