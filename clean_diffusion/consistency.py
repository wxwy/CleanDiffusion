#!/usr/bin/env python3
"""Teacher-free Consistency + CFG 的端到端教学实现。

这个文件故意采用 CleanRL-style 单文件结构：读者打开一个文件，就能顺着
配置 -> 数据 -> 模型 -> Consistency endpoint objective -> one-step sampler -> train/sample 主流程读完。

当前文件实现阶段 3：

- Teacher-free endpoint consistency objective
- Consistency one-step sampler
...
python clean_diffusion/consistency.py train \
...
python clean_diffusion/consistency.py sample \
  --ckpt-root checkpoints/consistency_smoke \
  --clip-root /path/to/clip \
  --output-dir samples/consistency_smoke \
  --image-size 32 \
  --base-channels 16 \
  --channel-mults 1,2,4 \
...
Teacher-free Consistency 的核心训练公式：

  x0 = data/image
  x1 = Gaussian noise
  t=0 -> noise, t=1 -> data
  x_t = (1 - t) * x1 + t * x0
  f_theta(x_t, t, text) -> x0_hat
  loss = MSE(f_theta(x_ta, ta), stopgrad(f_ema(x_tb, tb)))
       + lambda_boundary * MSE(f_theta(x0, 1), x0)

采样时从 Gaussian noise 出发，在 t=0 一步预测 endpoint x0_hat。EMA 是当前模型的滑动平均，不是外部 pretrained teacher。
"""
...
def _early_help_if_requested() -> None:
    ...
    if len(sys.argv) == 1 or sys.argv[1] in {"-h", "--help"}:
        print(
            "CleanDiffusion Teacher-free Consistency single-file baseline\n\n"
            "Usage:\n"
            "  python clean_diffusion/consistency.py train [options]\n"
            "  python clean_diffusion/consistency.py sample [options]\n\n"
            "Run subcommand help:\n"
            "  python clean_diffusion/consistency.py train --help\n"
            "  python clean_diffusion/consistency.py sample --help"
        )
        raise SystemExit(0)
    if len(sys.argv) >= 3 and sys.argv[2] in {"-h", "--help"}:
        if sys.argv[1] == "train":
            print(
                "Train Teacher-free Consistency\n\n"
                "Required:\n"
                "  --coco-img-root PATH\n"
                "  --coco-ann-file PATH\n"
                "  --clip-root PATH\n\n"
                ...
                "  --max-steps 1 --sample-scheduler consistency --sample-steps 4 --no-amp"
            )
            raise SystemExit(0)
        if sys.argv[1] == "sample":
            print(
                "Sample from Consistency checkpoint\n\n"
                "Required:\n"
                "  --ckpt-root PATH\n"
                "  --clip-root PATH\n\n"
                "Common:\n"
                "  --output-dir samples --image-size 32 --base-channels 16 --channel-mults 1,2,4\n"
                "  --scheduler consistency --steps 4 --guidance-scales 3.0 --prompts \"a small red car\""
            )
            raise SystemExit(0)
...
class TrainConfig:
    ...
    sample_scheduler: str = "consistency"
    ...
    def resolved_run_name(self) -> str:
        ...
        return sanitize_name(f"consistency_unet_{time.strftime('%Y%m%d_%H%M%S')}{suffix}")
...
# =============================================================================
# 5. Teacher-free Consistency objective：直接预测 endpoint x0_hat。
# =============================================================================
...
def consistency_loss(
    ...
    """teacher-free consistency：同一条直线路径上的 endpoint 预测应一致。

    模型直接输出 endpoint x0_hat。EMA target 是当前模型的滑动平均副本，
    用来稳定训练，不是外部 pretrained teacher。
    """
    b = x0.size(0)
    x1 = torch.randn_like(x0)
    t_a = torch.rand(b, device=x0.device)
    t_b = torch.rand(b, device=x0.device)
    x_ta = (1.0 - expand_time(t_a, x0)) * x1 + expand_time(t_a, x0) * x0
    x_tb = (1.0 - expand_time(t_b, x0)) * x1 + expand_time(t_b, x0) * x0

    pred_endpoint = model(x_ta, t_a, text_context)
    with torch.no_grad():
        target_endpoint = ema_model(x_tb, t_b, text_context)
    loss_consistency = F.mse_loss(pred_endpoint, target_endpoint)

    t_data = torch.ones(b, device=x0.device)
    boundary_endpoint = model(x0, t_data, text_context)
    loss_boundary = F.mse_loss(boundary_endpoint, x0)
    return loss_consistency + boundary_weight * loss_boundary


# =============================================================================
# 6. CFG + consistency sampler：从噪声一步预测 endpoint。
# =============================================================================
...
def sample_with_scheduler(
    ...
    model.eval()
    if scheduler == "consistency":
        return consistency_sample(model, device, cond_context, uncond_context, guidance_scale, shape)
    raise ValueError(f'scheduler should be "consistency", got {scheduler}')
...
def train(cfg: TrainConfig) -> None:
    if cfg.sample_scheduler != "consistency":
        raise ValueError(f'当前 Consistency 文件只支持 sample_scheduler="consistency"，got {cfg.sample_scheduler}')
        ...
        for epoch in range(cfg.epochs):
            for images, text_tokens, captions, cond_captions, is_dropped in dataloader:
                ...
                with torch.amp.autocast(device_type=device.type, enabled=cfg.amp and device.type == "cuda"):
                    loss = consistency_loss(model, ema_model, images, text_context, cfg.boundary_weight)
...
def sample(cfg: SampleConfig) -> None:
    if cfg.scheduler != "consistency":
        raise ValueError(f'当前 Consistency 文件只支持 scheduler="consistency"，got {cfg.scheduler}')
    device = get_device(cfg.device)
...
def add_train_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--sample-scheduler", choices=["consistency"], default="consistency")
    ...
    parser.add_argument("--sample-eta", type=float, default=0.0, help="保留参数位；consistency one-step 当前不使用 eta")
...
def add_sample_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--scheduler", choices=["consistency"], default="consistency")
    parser.add_argument("--steps", type=int, default=50)
    parser.add_argument("--eta", type=float, default=0.0, help="保留参数位；consistency one-step 当前不使用 eta")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="CleanDiffusion Teacher-free Consistency single-file baseline")
    subparsers = parser.add_subparsers(dest="command", required=True)
    train_parser = subparsers.add_parser("train", help="train Teacher-free Consistency")
    add_train_args(train_parser)
    sample_parser = subparsers.add_parser("sample", help="sample from a Consistency checkpoint")
