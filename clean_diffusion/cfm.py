#!/usr/bin/env python3
"""Consistency Flow Matching + CFM Euler + CFG 的端到端教学实现。

这个文件故意采用 CleanRL-style 单文件结构：读者打开一个文件，就能顺着
配置 -> 数据 -> 模型 -> CFM velocity+endpoint objective -> CFM Euler sampler -> train/sample 主流程读完。

当前文件实现阶段 4：

- Flow Matching velocity objective + endpoint consistency
...
python clean_diffusion/cfm.py train \
...
python clean_diffusion/cfm.py sample \
  --ckpt-root checkpoints/cfm_smoke \
  --clip-root /path/to/clip \
  --output-dir samples/cfm_smoke \
  --image-size 32 \
  --base-channels 16 \
  --channel-mults 1,2,4 \
  --scheduler cfm \
...
Consistency Flow Matching 的核心训练公式：
  ...
  x_t = (1 - t) * x1 + t * x0
  v_target = x0 - x1
  x0_hat = x_t + (1 - t) * v_theta(x_t, t, text)
  loss = MSE(v_theta, v_target) + lambda_cons * endpoint_consistency

采样时从 Gaussian noise 出发，沿速度场用 CFM Euler 积分到 t=1。
"""
...
def _early_help_if_requested() -> None:
    ...
    if len(sys.argv) == 1 or sys.argv[1] in {"-h", "--help"}:
        print(
            "CleanDiffusion Consistency Flow Matching single-file baseline\n\n"
            "Usage:\n"
            "  python clean_diffusion/cfm.py train [options]\n"
            "  python clean_diffusion/cfm.py sample [options]\n\n"
            "Run subcommand help:\n"
            "  python clean_diffusion/cfm.py train --help\n"
            "  python clean_diffusion/cfm.py sample --help"
        )
        raise SystemExit(0)
    if len(sys.argv) >= 3 and sys.argv[2] in {"-h", "--help"}:
        if sys.argv[1] == "train":
            print(
                ...
                "  --max-steps 1 --sample-scheduler cfm --sample-steps 4 --no-amp"
            )
            raise SystemExit(0)
        if sys.argv[1] == "sample":
            print(
                "Sample from CFM checkpoint\n\n"
                "Required:\n"
                "  --ckpt-root PATH\n"
                "  --clip-root PATH\n\n"
                "Common:\n"
                "  --output-dir samples --image-size 32 --base-channels 16 --channel-mults 1,2,4\n"
                "  --scheduler cfm --steps 4 --guidance-scales 3.0 --prompts \"a small red car\""
            )
            raise SystemExit(0)
...
class TrainConfig:
    ...
    def resolved_run_name(self) -> str:
        ...
        return sanitize_name(f"cfm_unet_{time.strftime('%Y%m%d_%H%M%S')}{suffix}")
...
# =============================================================================
# 5. CFM objective：速度场学习 + endpoint consistency。
# =============================================================================
...
def endpoint_from_velocity(model: nn.Module, x_t: torch.Tensor, t: torch.Tensor, text_context: torch.Tensor) -> torch.Tensor:
    v = model(x_t, t, text_context)
    return x_t + (1.0 - expand_time(t, x_t)) * v
...
def cfm_loss(
    ...
    """CFM = Flow Matching velocity loss + endpoint consistency loss."""
    b = x0.size(0)
    x1 = torch.randn_like(x0)
    t = torch.rand(b, device=x0.device)
    x_t = (1.0 - expand_time(t, x0)) * x1 + expand_time(t, x0) * x0
    v_target = x0 - x1
    v_pred = model(x_t, t, text_context)
    ...
    pred_endpoint = endpoint_from_velocity(model, x_ta, t_a, text_context)
    with torch.no_grad():
        target_endpoint = endpoint_from_velocity(ema_model, x_tb, t_b, text_context)
    loss_consistency = F.mse_loss(pred_endpoint, target_endpoint)

    t_data = torch.ones(b, device=x0.device)
    boundary_endpoint = endpoint_from_velocity(model, x0, t_data, text_context)
    loss_boundary = F.mse_loss(boundary_endpoint, x0)
    return loss_flow + consistency_weight * loss_consistency + boundary_weight * loss_boundary


# =============================================================================
# 6. CFG + CFM Euler sampler：沿速度场从噪声走到数据。
# =============================================================================


@torch.no_grad()
def cfg_predict_velocity(
    model: nn.Module,
    x: torch.Tensor,
    t: torch.Tensor,
    ...
    v_uncond = model(x, t, uncond_context)
    v_cond = model(x, t, cond_context)
    return v_uncond + guidance_scale * (v_cond - v_uncond)


@torch.no_grad()
def cfm_euler_sample(
    model: nn.Module,
    device: torch.device,
    cond_context: torch.Tensor,
    uncond_context: torch.Tensor,
    ...
    """从 t=0 的 Gaussian noise 出发，沿 dx/dt=v_theta(x,t) 积分到 t=1。"""
    if steps <= 0:
    ...
    for i in range(steps):
        ...
        v = cfg_predict_velocity(model, x, t, cond_context, uncond_context, guidance_scale)
        x = x + dt * v
    return x
...
def sample_with_scheduler(
    ...
    model.eval()
    if scheduler == "cfm":
        return cfm_euler_sample(model, device, cond_context, uncond_context, guidance_scale, steps, shape)
    raise ValueError(f'scheduler should be "cfm", got {scheduler}')
...
def save_checkpoint(
    ...
    metadata = {
        ...
        "lambda_cons": cfg.consistency_weight,
...
def train(cfg: TrainConfig) -> None:
    if cfg.sample_scheduler != "cfm":
        raise ValueError(f'当前 CFM 文件只支持 sample_scheduler="cfm"，got {cfg.sample_scheduler}')
        ...
        for epoch in range(cfg.epochs):
            for images, text_tokens, captions, cond_captions, is_dropped in dataloader:
                ...
                with torch.amp.autocast(device_type=device.type, enabled=cfg.amp and device.type == "cuda"):
                    loss = cfm_loss(model, ema_model, images, text_context, cfg.consistency_weight, cfg.boundary_weight)
...
def sample(cfg: SampleConfig) -> None:
    if cfg.scheduler != "cfm":
        raise ValueError(f'当前 CFM 文件只支持 scheduler="cfm"，got {cfg.scheduler}')
    device = get_device(cfg.device)
...
def add_train_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--sample-scheduler", choices=["cfm"], default="cfm")
    ...
    parser.add_argument("--sample-eta", type=float, default=0.0, help="保留参数位；CFM Euler 当前不使用 eta")
...
def add_sample_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--scheduler", choices=["cfm"], default="cfm")
    parser.add_argument("--steps", type=int, default=50)
    parser.add_argument("--eta", type=float, default=0.0, help="保留参数位；CFM Euler 当前不使用 eta")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="CleanDiffusion Consistency Flow Matching single-file baseline")
    subparsers = parser.add_subparsers(dest="command", required=True)
    train_parser = subparsers.add_parser("train", help="train Consistency Flow Matching")
    add_train_args(train_parser)
    sample_parser = subparsers.add_parser("sample", help="sample from a CFM checkpoint")
