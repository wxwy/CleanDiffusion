#!/usr/bin/env python3
"""DMD-lite one-step distillation + CFG 的端到端教学实现。

这个文件故意采用 CleanRL-style 单文件结构：读者打开一个文件，就能顺着
配置 -> 数据 -> 模型 -> DMD-lite student objective -> one-step sampler -> train/sample 主流程读完。

当前文件实现阶段 5：

- One-step student distillation from an FM/RF/CFM teacher
...
python clean_diffusion/dmd_lite.py train \
...
python clean_diffusion/dmd_lite.py sample \
  --ckpt-root checkpoints/dmd_lite_smoke \
  --clip-root /path/to/clip \
  --output-dir samples/dmd_lite_smoke \
  --image-size 32 \
  --base-channels 16 \
  --channel-mults 1,2,4 \
  --scheduler dmd_lite \
...
DMD-lite 的核心训练公式：
  ...
  t=0 -> noise, t=1 -> data
  x_t = (1 - t) * x1 + t * x0
  teacher: 已有 FM/RF/CFM checkpoint，用 Euler 生成 target endpoint
  student: f_theta(x1, t=0, text) -> x0_hat
  loss = MSE(student_endpoint, stopgrad(teacher_endpoint))

采样时 student 从 Gaussian noise 出发，在 t=0 一步预测 endpoint x0_hat。这里不实现完整 adversarial distribution matching。
"""
...
def _early_help_if_requested() -> None:
    ...
    if len(sys.argv) == 1 or sys.argv[1] in {"-h", "--help"}:
        print(
            "CleanDiffusion DMD-lite single-file baseline\n\n"
            "Usage:\n"
            "  python clean_diffusion/dmd_lite.py train [options]\n"
            "  python clean_diffusion/dmd_lite.py sample [options]\n\n"
            "Run subcommand help:\n"
            "  python clean_diffusion/dmd_lite.py train --help\n"
            "  python clean_diffusion/dmd_lite.py sample --help"
        )
        raise SystemExit(0)
    if len(sys.argv) >= 3 and sys.argv[2] in {"-h", "--help"}:
        if sys.argv[1] == "train":
            print(
                ...
                "  --max-steps 1 --sample-scheduler dmd_lite --sample-steps 4 --no-amp"
            )
            raise SystemExit(0)
        if sys.argv[1] == "sample":
            print(
                "Sample from DMD-lite checkpoint\n\n"
                "Required:\n"
                "  --ckpt-root PATH\n"
                "  --clip-root PATH\n\n"
                "Common:\n"
                "  --output-dir samples --image-size 32 --base-channels 16 --channel-mults 1,2,4\n"
                "  --scheduler dmd_lite --steps 4 --guidance-scales 3.0 --prompts \"a small red car\""
            )
            raise SystemExit(0)
...
class TrainConfig:
    ...
    def resolved_run_name(self) -> str:
        ...
        return sanitize_name(f"dmd_lite_unet_{time.strftime('%Y%m%d_%H%M%S')}{suffix}")
...
# =============================================================================
# 5. DMD-lite objective：student 一步学习 teacher endpoint。
# =============================================================================
...
def dmd_lite_loss(
    ...
    """student 一步模仿 teacher 多步生成结果。

    images 只提供 batch shape 和 text pairing；target 来自 frozen teacher 从 noise
    生成的 endpoint，不使用 adversarial distribution matching。
    """
    noise = torch.randn_like(images)
    with torch.no_grad():
        target_endpoint = teacher_velocity_endpoint(teacher_model, noise, text_context, teacher_steps)
    t0 = torch.zeros((images.size(0),), device=images.device)
    student_endpoint = student_model(noise, t0, text_context)
    return F.mse_loss(student_endpoint, target_endpoint)
...
def sample_with_scheduler(
    ...
    model.eval()
    if scheduler == "dmd_lite":
        return dmd_lite_sample(model, device, cond_context, uncond_context, guidance_scale, shape)
    raise ValueError(f'scheduler should be "dmd_lite", got {scheduler}')
...
def train(cfg: TrainConfig) -> None:
    if cfg.sample_scheduler != "dmd_lite":
        raise ValueError(f'当前 DMD-lite 文件只支持 sample_scheduler="dmd_lite"，got {cfg.sample_scheduler}')
    if not cfg.teacher_ckpt_root:
        raise ValueError("--teacher-ckpt-root 不能为空；DMD-lite 需要一个已有 FM/RF/CFM teacher checkpoint")
        ...
        for epoch in range(cfg.epochs):
            for images, text_tokens, captions, cond_captions, is_dropped in dataloader:
                ...
                with torch.amp.autocast(device_type=device.type, enabled=cfg.amp and device.type == "cuda"):
                    loss = dmd_lite_loss(model, teacher_model, images, text_context, cfg.teacher_steps)
...
def sample(cfg: SampleConfig) -> None:
    if cfg.scheduler != "dmd_lite":
        raise ValueError(f'当前 DMD-lite 文件只支持 scheduler="dmd_lite"，got {cfg.scheduler}')
    device = get_device(cfg.device)
...
def add_train_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--sample-scheduler", choices=["dmd_lite"], default="dmd_lite")
    ...
    parser.add_argument("--sample-eta", type=float, default=0.0, help="保留参数位；DMD-lite one-step 当前不使用 eta")
...
def add_sample_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--scheduler", choices=["dmd_lite"], default="dmd_lite")
    parser.add_argument("--steps", type=int, default=50)
    parser.add_argument("--eta", type=float, default=0.0, help="保留参数位；DMD-lite one-step 当前不使用 eta")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="CleanDiffusion DMD-lite single-file baseline")
    subparsers = parser.add_subparsers(dest="command", required=True)
    train_parser = subparsers.add_parser("train", help="train DMD-lite")
    add_train_args(train_parser)
    sample_parser = subparsers.add_parser("sample", help="sample from a DMD-lite checkpoint")
