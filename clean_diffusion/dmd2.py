#!/usr/bin/env python3
"""DMD2-style one-step student experiment + CFG 的端到端教学实现。

这个文件故意采用 CleanRL-style 单文件结构：读者打开一个文件，就能顺着
配置 -> 数据 -> 模型 -> DMD2-style score/GAN objective -> one-step sampler -> train/sample 主流程读完。

当前文件实现阶段 6：

- One-step student with fake score model and discriminator proxy
- One-step student sampler
...
python clean_diffusion/dmd2.py train \
...
python clean_diffusion/dmd2.py sample \
  --ckpt-root checkpoints/dmd2_smoke \
  --clip-root /path/to/clip \
  --output-dir samples/dmd2_smoke \
  --image-size 32 \
  --base-channels 16 \
  --channel-mults 1,2,4 \
  --scheduler dmd2 \
...
DMD2-style 教学实验的核心结构：

  x0 = data/image
  x1 = Gaussian noise
  t=0 -> noise, t=1 -> data
  x_t = (1 - t) * x1 + t * x0
  teacher: 已有 FM/RF/CFM checkpoint，提供 reference endpoint / score direction
  fake_score_model: 追踪当前 student 生成分布的 score proxy
  discriminator: 区分 real image 与 student fake image
  student: 用 score-delta surrogate + GAN proxy 更新

这是 DMD2-style 教学骨架，不是完整论文复现。采样时 student 一步预测 endpoint。
"""
...
def _early_help_if_requested() -> None:
    ...
    if len(sys.argv) == 1 or sys.argv[1] in {"-h", "--help"}:
        print(
            "CleanDiffusion DMD2-style single-file baseline\n\n"
            "Usage:\n"
            "  python clean_diffusion/dmd2.py train [options]\n"
            "  python clean_diffusion/dmd2.py sample [options]\n\n"
            "Run subcommand help:\n"
            "  python clean_diffusion/dmd2.py train --help\n"
            "  python clean_diffusion/dmd2.py sample --help"
        )
        raise SystemExit(0)
    if len(sys.argv) >= 3 and sys.argv[2] in {"-h", "--help"}:
        if sys.argv[1] == "train":
            print(
                "Train DMD2-style experiment\n\n"
                "Required:\n"
                "  --coco-img-root PATH\n"
                "  --coco-ann-file PATH\n"
                "  --clip-root PATH\n\n"
                ...
                "  --teacher-ckpt-root checkpoints/cfm_smoke --teacher-steps 4\n"
                "  --image-size 32 --batch-size 2 --base-channels 16 --channel-mults 1,2,4\n"
                "  --max-steps 1 --sample-scheduler dmd2 --sample-steps 4 --no-amp"
            )
            raise SystemExit(0)
        if sys.argv[1] == "sample":
            print(
                "Sample from DMD2-style checkpoint\n\n"
                "Required:\n"
                "  --ckpt-root PATH\n"
                "  --clip-root PATH\n\n"
                "Common:\n"
                "  --output-dir samples --image-size 32 --base-channels 16 --channel-mults 1,2,4\n"
                "  --scheduler dmd2 --steps 4 --guidance-scales 3.0 --prompts \"a small red car\""
            )
            raise SystemExit(0)
...
class TrainConfig:
    ...
    sample_scheduler: str = "dmd2"
    teacher_ckpt_root: str = ""
    teacher_steps: int = 4
    ...
    fake_score_updates: int = 1
    discriminator_updates: int = 1
    fake_score_lr: float = 0.0
    ...
    def resolved_run_name(self) -> str:
        ...
        return sanitize_name(f"dmd2_unet_{time.strftime('%Y%m%d_%H%M%S')}{suffix}")
...
# =============================================================================
# 5. DMD2-style objective：fake score + discriminator + score-delta surrogate。
# =============================================================================
...
def teacher_velocity_endpoint(
    teacher_model: nn.Module,
    noise: torch.Tensor,
    text_context: torch.Tensor,
    steps: int,
) -> torch.Tensor:
    """用 FM/RF/CFM teacher 的速度场生成 endpoint target。"""
    if steps <= 0:
        raise ValueError(f"teacher_steps must be positive, got {steps}")
    x = noise
    dt = 1.0 / steps
    for i in range(steps):
        t = torch.full((noise.size(0),), i * dt, device=noise.device)
        v = teacher_model(x, t, text_context)
        x = x + dt * v
    return x


def student_endpoint(student_model: nn.Module, noise: torch.Tensor, text_context: torch.Tensor) -> torch.Tensor:
    t0 = torch.zeros((noise.size(0),), device=noise.device)
    return student_model(noise, t0, text_context)


def discriminator_loss(discriminator: nn.Module, real: torch.Tensor, fake: torch.Tensor) -> torch.Tensor:
...
def generator_gan_loss(discriminator: nn.Module, fake: torch.Tensor) -> torch.Tensor:
    fake_logits = discriminator(fake)
    return F.binary_cross_entropy_with_logits(fake_logits, torch.ones_like(fake_logits))


def score_delta_surrogate_loss(fake: torch.Tensor, real_score: torch.Tensor, fake_score: torch.Tensor) -> torch.Tensor:
    score_delta = fake_score - real_score
    score_delta = score_delta / score_delta.flatten(1).norm(dim=1).clamp_min(1e-6).view(-1, 1, 1, 1)
    target = (fake - score_delta).detach()
    return 0.5 * F.mse_loss(fake, target)


# =============================================================================
# 6. CFG + one-step student sampler：从噪声一步预测 endpoint。
# =============================================================================


@torch.no_grad()
def cfg_predict_endpoint(
    model: nn.Module,
    x: torch.Tensor,
    t: torch.Tensor,
    ...
    x0_uncond = model(x, t, uncond_context)
    x0_cond = model(x, t, cond_context)
    return x0_uncond + guidance_scale * (x0_cond - x0_uncond)
...
def dmd2_sample(
    ...
    """从 t=0 的 Gaussian noise 出发，一步预测 endpoint x0_hat。"""
    b = shape[0]
    x = torch.randn(shape, device=device)
    t = torch.zeros((b,), device=device)
    return cfg_predict_endpoint(model, x, t, cond_context, uncond_context, guidance_scale)
...
def sample_with_scheduler(
    ...
    model.eval()
    if scheduler == "dmd2":
        return dmd2_sample(model, device, cond_context, uncond_context, guidance_scale, shape)
    raise ValueError(f'scheduler should be "dmd2", got {scheduler}')
    ...
    if extra_modules:
        for name, module in extra_modules.items():
            ...
            if path.exists():
                ...
                log(f"已加载 {name}")
    if extra_optimizers:
        for name, extra_optimizer in extra_optimizers.items():
            path = folder / f"{name}.pt"
            if path.exists():
                extra_optimizer.load_state_dict(torch.load(path, map_location="cpu"))
                log(f"已加载 {name}")
...
def save_checkpoint(
    ...
    if extra_modules:
        for name, module in extra_modules.items():
            torch.save(module.state_dict(), folder / f"{name}.pt")
    if extra_optimizers:
        for name, extra_optimizer in extra_optimizers.items():
            torch.save(extra_optimizer.state_dict(), folder / f"{name}.pt")
    ...
    metadata = {
        ...
        "lambda_cons": 0.0,
        "teacher_ckpt_root": cfg.teacher_ckpt_root,
...
def sample_for_training_log(
    ...
    uncond_context = encode_text_from_prompts(clip_model, [""] * sample_size, device)
...
def train(cfg: TrainConfig) -> None:
    if cfg.sample_scheduler != "dmd2":
        raise ValueError(f'当前 DMD2 文件只支持 sample_scheduler="dmd2"，got {cfg.sample_scheduler}')
    if not cfg.teacher_ckpt_root:
        raise ValueError("--teacher-ckpt-root 不能为空；DMD2-style 需要一个已有 FM/RF/CFM teacher checkpoint")
    if cfg.amp:
        raise ValueError("DMD2-style 当前教学实现请使用 --no-amp")
    ...
    fake_score_model = copy.deepcopy(model).to(device)
    discriminator = PatchDiscriminator().to(device)
    teacher_model = build_model_from_config(cfg, device)
    teacher_ema = copy.deepcopy(teacher_model)
    load_checkpoint(teacher_model, optimizer=None, scaler=None, ema_model=teacher_ema, ckpt_root=cfg.teacher_ckpt_root, step="last")
    teacher_model = teacher_ema.to(device).eval()
    for p in teacher_model.parameters():
        p.requires_grad = False
    dataloader = build_dataloader(cfg)

    optimizer = torch.optim.Adam(model.parameters(), lr=cfg.lr)
    fake_optimizer = torch.optim.Adam(fake_score_model.parameters(), lr=cfg.fake_score_lr or cfg.lr)
    discriminator_optimizer = torch.optim.Adam(discriminator.parameters(), lr=cfg.discriminator_lr or cfg.lr)
    scaler = torch.amp.GradScaler(device.type, enabled=cfg.amp and device.type == "cuda")
    loss_list, global_step = load_checkpoint(
        model,
        optimizer,
        scaler,
        ema_model,
        ...
        step="last",
        extra_modules={
            "fake_score_model": fake_score_model,
    ...
    try:
        ...
        for epoch in range(cfg.epochs):
            for images, text_tokens, captions, cond_captions, is_dropped in dataloader:
                ...
                for _ in range(cfg.fake_score_updates):
                    noise = torch.randn_like(images)
                    with torch.no_grad():
                        fake = student_endpoint(model, noise, text_context)
                        target_v = fake - noise
                    t0 = torch.zeros((images.size(0),), device=device)
                    fake_pred_v = fake_score_model(noise, t0, text_context)
                    fake_loss = F.mse_loss(fake_pred_v, target_v)
                    fake_optimizer.zero_grad(set_to_none=True)
                    fake_loss.backward()
                    torch.nn.utils.clip_grad_norm_(fake_score_model.parameters(), cfg.grad_clip)
                    fake_optimizer.step()
                ...
                for _ in range(cfg.discriminator_updates):
                    ...
                    with torch.no_grad():
                        fake = student_endpoint(model, noise, text_context)
                    disc_loss = discriminator_loss(discriminator, images, fake)
                    discriminator_optimizer.zero_grad(set_to_none=True)
                    disc_loss.backward()
                    torch.nn.utils.clip_grad_norm_(discriminator.parameters(), cfg.grad_clip)
                    discriminator_optimizer.step()

                noise = torch.randn_like(images)
                fake = student_endpoint(model, noise, text_context)
                with torch.no_grad():
                    real_endpoint = teacher_velocity_endpoint(teacher_model, noise, text_context, cfg.teacher_steps)
                    real_score = real_endpoint - noise
                    t0 = torch.zeros((images.size(0),), device=device)
                    fake_score = fake_score_model(noise, t0, text_context)
                loss_dmd = score_delta_surrogate_loss(fake, real_score, fake_score)
                loss_gan = generator_gan_loss(discriminator, fake)
                loss = cfg.dmd_weight * loss_dmd + cfg.gan_weight * loss_gan
                ...
                if global_step % cfg.sample_every == 0:
                    ...
                    sample_for_training_log(ema_model, device, images, text_context, clip_model, cfg, global_step, writer)
                    model.train()
...
def sample(cfg: SampleConfig) -> None:
    if cfg.scheduler != "dmd2":
        raise ValueError(f'当前 DMD2 文件只支持 scheduler="dmd2"，got {cfg.scheduler}')
    device = get_device(cfg.device)
...
def add_train_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--sample-scheduler", choices=["dmd2"], default="dmd2")
    parser.add_argument("--teacher-ckpt-root", default="")
    ...
    parser.add_argument("--fake-score-updates", type=int, default=1)
    parser.add_argument("--discriminator-updates", type=int, default=1)
    parser.add_argument("--fake-score-lr", type=float, default=0.0)
    parser.add_argument("--discriminator-lr", type=float, default=0.0)
    parser.add_argument("--sample-every", type=int, default=2500)
    parser.add_argument("--save-every", type=int, default=5000)
    parser.add_argument("--print-every", type=int, default=100)
    parser.add_argument("--guidance-scale", type=float, default=5.0)
    parser.add_argument("--sample-steps", type=int, default=50)
    parser.add_argument("--sample-eta", type=float, default=0.0, help="保留参数位；DMD2-style one-step 当前不使用 eta")
...
def add_sample_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--scheduler", choices=["dmd2"], default="dmd2")
    parser.add_argument("--steps", type=int, default=50)
    parser.add_argument("--eta", type=float, default=0.0, help="保留参数位；DMD2-style one-step 当前不使用 eta")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="CleanDiffusion DMD2-style single-file baseline")
    subparsers = parser.add_subparsers(dest="command", required=True)
    train_parser = subparsers.add_parser("train", help="train DMD2-style experiment")
    add_train_args(train_parser)
    sample_parser = subparsers.add_parser("sample", help="sample from a DMD2-style checkpoint")
